...
>
> The camera stack, VFX prefabs, prewarm list and scene extras were regenerated
> in the required builder order on 2026-08-13. The three surface layers remain
...
- `Kit_Weapon_Default` — one viewmodel prefab and explicit material.
- `Kit_Enemy_Default` — one prefab per drone archetype and one shared hull material.
...
named `Art`; every collider in that subtree is destroyed. Arena collision and
NavMesh therefore still come from the same transformed `BoxCollider`, drone hit
geometry still comes from the generated hull/core, and the weapon's muzzle,
casing-eject point, sway and firing logic stay on the generated rig.

`Texture_Art_1024.preset` and `Model_Art_Static.preset` live under
`Assets/_Project/Art/Presets/`. `ArtImportPostprocessor` applies them before
import to `Assets/_Project/Art/Imported/` and `Assets/ThirdParty/`: 1024 max size,
streaming mipmaps, compressed and non-readable textures; non-readable,
medium-compressed models with cameras/lights/material extraction disabled.
...
**G8 measurement (2026-08-13):** zero new runtime textures and therefore **0 MB
...
live film grain, countdown and moving enemies. A second capture of the unchanged
post-G8 binary produced the same variance envelope (static arena mean absolute
difference `0.03–0.59 / 255`; wave frames vary with enemy position), so no visual
delta is attributable to the null-kit seam.
