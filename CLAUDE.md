...
**Phases 0-7 are code-complete: the whole loop exists.** Unity 6000.0.81f1 + URP,
Personal licence active. The grey box has the rifle, three drone archetypes,
timed waves feeding a shop, permadeath with a saved best round, and four stacking
effect modules.
...
| Quality gates | ✅ `node Tools/typecheck.mjs` (7 assemblies) + `node Tools/check.mjs` + **42 tests** (37 EditMode, 5 PlayMode) all green |
| Phase 3 — grey box | ✅ movement, rifle, viewmodel, pooling, impact feedback, HUD, cheat console — **verified in play** |
| Phase 4 — Rusher | ✅ NavMesh pathing, fused contact detonation, player damage feedback — compiled, **not yet play-tested** |
| Phase 5 — waves/shop/save | ✅ wave loop, attack tokens, passives, versioned JSON save — compiled, **not yet play-tested** |
| Phase 6 — Shooter + Tank | ✅ ranged bursts with a deliberate opening miss, telegraphed slam — compiled, **not yet play-tested** |
| Phase 7 — effect modules | ✅ explosive / pierce / ricochet / chain with depth rules — compiled, **not yet play-tested** |
...
headlessly, has every scene reference proven by a save/reload round trip, and now
*runs under test*: the PlayMode suite loads the grey box, waits for the wave to
start, and asserts drones spawn, path toward the player, take damage, pay out,
and hand the run to the shop with the pool recycling. So "nobody has run it" is
no longer true.

What is still open is the only thing a machine cannot answer: **is it fun.** The
tuning card at the top of [docs/NEXT-SESSION-PROMPT.md](docs/NEXT-SESSION-PROMPT.md)
is seven things to feel, each naming the asset field to move. How autopilot
operates between those sessions — and what it may decide alone — is
