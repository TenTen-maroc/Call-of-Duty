...
| | |
| --- | --- |
| Repo foundation | ✅ LFS, eight guards, pre-commit hook via `core.hooksPath` |
| Scenes + prefabs | ✅ generated by `CoD → Build Grey Box`, self-verified by `GreyBoxVerify` |
| Quality gates | ✅ `node Tools/typecheck.mjs` (9 assemblies, tests included) + `node Tools/check.mjs` + **287 tests** + `node Tools/verify-build.mjs` + `node Tools/screenshot.mjs` |
...
| D14 — missions 1-2 | ⚠️ SHAKEDOWN and HARD CONTACT. Review found **four separate ways a mission could be uncompletable** before these ran — a fifth turned up in A1 — [docs/systems/campaign.md](docs/systems/campaign.md) — and they are still **never played** |
...
| G5a — the audio mixer | ✅ ten buses, four exposed parameters, footsteps and ambience routed, and a **gate** for the one asset no builder can make — [docs/systems/audio.md](docs/systems/audio.md). Groups are hand-writable as YAML; **effects are not** (the DSP needs GUIDs only the editor mints) so the Reverb bus is empty and is three editor clicks. Hitmarkers, dry-fire and player-hurt remain synthetic placeholders |
| G8 — the art seam | ✅ optional arena / weapon / enemy kit assets, collider-stripped `Art` children, primitive fallback, all-null-or-all-complete verification, and importer presets. G8 shipped all-null and visually identical — [docs/systems/rendering.md](docs/systems/rendering.md) |
| G9a — ambientCG | ✅ ten CC0 industrial surfaces at 1K; only color + NormalGL retained, arena kit complete, **19.9 MB** measured texture-memory delta — [docs/systems/rendering.md](docs/systems/rendering.md) |
| G9a — Poly Haven | ✅ Autoshop 01 CC0 HDRI imported as a **128 px**, linear, specular-convolved reflection cubemap; procedural sky remains visible, **0.2 MB** measured texture-memory delta — [docs/systems/rendering.md](docs/systems/rendering.md) |
| G9a — Kenney | ✅ 18 retained CC0 clips for footsteps, impacts, facility ambience, enemies/explosions and interface feedback; optional `AudioKitConfig`, **0 MB VRAM / 0.8 MB audio memory** — [docs/systems/audio.md](docs/systems/audio.md) |
| G9a — Sonniss | ✅ three trimmed royalty-free firearm clips replace the generated close/tail/reload layers on all eight weapons; exact range extraction from the 8.4 GB archive, **0 MB VRAM / +0.4 MB audio memory** — [docs/systems/audio.md](docs/systems/audio.md) |
| H1 — Mission 1 humanization | ✅ Mara Venn's data-driven, null-audio-safe radio subtitles; contextual objectives; six pooled drone reactions; a collider-free last-stand corner; and a four-second post-combat quiet beat — machine-verified, never human-auditioned — [docs/systems/campaign.md](docs/systems/campaign.md) |
| H2 — Mission 2 human combat | ✅ `11_AtlasOutpost`, visible Humanoid Meridian Riflemen, bounded cover, anatomical hits, ragdoll/dismemberment, schema-6 violence settings, outdoor audio/VFX, and a nine-frame built-player visual route — machine-verified and visually reviewed; feel/audio/RTX-3050 timing remain human gates — [docs/systems/humans.md](docs/systems/humans.md) |
| F1 — feel and image pass | ⚠️ the Rifleman's headshot breakpoint (115 → 110), an arena dimmed below its own telegraph, hitstop on kill, a Tank core finally worth aiming at, and the first verticality the arena has ever had. Compiled, guarded and gated by 20 new tests — **never played, and the lighting half cannot be judged by any machine** — [docs/systems/player.md](docs/systems/player.md), [arena.md](docs/systems/arena.md), [rendering.md](docs/systems/rendering.md) |
| A1 — audit close-out | ⚠️ the fifth way a mission could be uncompletable (`_zones` was wired and nothing proved it stayed wired — `GreyBoxVerify.CheckZones`, plus the one PlayMode test that registers no zone of its own), and a campaign checkpoint that rewound one wave short from the second wave group onward (`WaveRunner.NextWaveNumber`) — [docs/systems/campaign.md](docs/systems/campaign.md). **Neither the gates nor the suites have been run: this container has no Unity and `download.unity3d.com` is blocked by egress policy** |
...
runs under test: 287 tests across both suites drive the real loop and the real
