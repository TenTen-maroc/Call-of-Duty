...
| Quality gates | ✅ `node Tools/typecheck.mjs` (9 assemblies, tests included) + `node Tools/check.mjs` + **237 tests** + `node Tools/verify-build.mjs` + `node Tools/screenshot.mjs` |
...
| G8 — the art seam | ✅ optional arena / weapon / enemy kit assets, collider-stripped `Art` children, primitive fallback, all-null-or-all-complete verification, and importer presets. The shipped kits are all null, so the rendered game is unchanged — [docs/systems/rendering.md](docs/systems/rendering.md) |
...
runs under test: 232 tests across both suites drive the real loop and the real
