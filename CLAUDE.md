...
**Phases 0-3 done: the grey box RUNS.** Unity 6000.0.81f1 + URP, Personal
licence active. Movement, look, hitscan rifle with recoil/bloom/reload
cancelling, 8-block viewmodel with sway, pooling for every spawn, impact
feedback, hitmarker, bloom-tracking crosshair, HUD, placeholder audio, and the
dev-gated cheat console are all in and verified in play.
...
| Quality gates | ✅ `node Tools/typecheck.mjs` + `node Tools/check.mjs` both green |
| 2026-08-11 audit | ✅ burst fire, weakpoint headshots, target respawn, pool double-despawn guard, casing physics, godmode wired |

**Before the next play session:** re-run **CoD → Build Grey Box** — the builder
gained the dummy head weakpoint, target respawn, and casing-layer changes after
the committed scenes were generated. Then follow
[docs/NEXT-SESSION-PROMPT.md](docs/NEXT-SESSION-PROMPT.md): play-test the newly
wired behaviours, then Phase 4 (the Rusher drone).
