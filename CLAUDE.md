...
**Phases 0-3 authored. Waiting on a Unity sign-in to generate scenes.**

| | |
| --- | --- |
| Repo foundation | ✅ `.gitignore`, `.gitattributes` + LFS, six guards, docs |
| Unity 6000.0.81f1 + IL2CPP | ✅ installed under `C:/Program Files/Unity/Hub/Editor/` |
| URP project | ✅ from the `urp-blank` template, merged at the repo root |
| Project settings | ✅ Force Text, Visible Meta Files, no domain/scene reload, Linear |
| Phase 2 asmdefs | ✅ `CoD.Core` / `Player` / `Weapons` / `UI` / `Editor` |
| Phase 3 scripts | ✅ authored, **never compiled** — no licence yet, so no compiler has seen them |
| Scenes + prefabs | ⏳ generated by `CoD → Build Grey Box`, which needs the editor running |
| Pre-commit hook | ✅ active via `core.hooksPath`, proven to block a `Library/` commit |
| `node Tools/check.mjs` | ✅ all six green |

**The one human step left:** sign in to Unity Hub with a Unity ID (Personal is
free). Nothing else is blocked. Then open the project and run **CoD → Build
Grey Box**, which creates every prefab, both scenes, and the tuning assets.

Expect first-compile errors — two dozen files went in without a compiler ever
seeing them. Fix those, then this is the grey box milestone: one gun in a grey
room, tuned until firing it is fun on its own.
