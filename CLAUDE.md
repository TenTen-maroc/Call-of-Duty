...
| Quality gates | ✅ `node Tools/typecheck.mjs` (9 assemblies, tests included) + `node Tools/check.mjs` + **244 tests** + `node Tools/verify-build.mjs` + `node Tools/screenshot.mjs` |
...
runs under test: 244 tests across both suites drive the real loop and the real
