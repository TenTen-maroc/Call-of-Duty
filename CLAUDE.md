...
| Quality gates | ✅ `node Tools/typecheck.mjs` (7 assemblies) + `node Tools/check.mjs` both green |
...
**What "code-complete" does not mean.** Everything above compiles clean, builds
headlessly, and has every scene reference proven by a save/reload round trip.
None of Phases 4-7 has been *played*. The milestone bar in this repo is FUN, not
working, and only the play session can answer that — see the tuning card in
[docs/NEXT-SESSION-PROMPT.md](docs/NEXT-SESSION-PROMPT.md), which lists the exact
asset field to move for each way it can feel wrong.
