# AGENTS.md — engineering rules for Call of Duty

> New here? This file is the contract for every Codex session and human
...
| Quality gates | ✅ `node Tools/typecheck.mjs` (9 assemblies, tests included) + `node Tools/check.mjs` + **257 tests** + `node Tools/verify-build.mjs` + `node Tools/screenshot.mjs` |
...
runs under test: 257 tests across both suites drive the real loop and the real
